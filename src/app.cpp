...
vec3 ray_col(const ray& r, const hittable_list& world, int depth) {
...
void Application::run() {
	...
	world.add(sphere(vec3(0, 0, -1000), 1000, mat_ground));
	...
	world.add(sphere(vec3(0, 0, 1), 1.0, mat1));

	auto mat2 = std::make_shared<lambertian>(vec3(0.4, 0.2, 0.1));
	world.add(sphere(vec3(4, 0, 1), 1.0, mat2));

	auto mat3 = std::make_shared<metal>(vec3(0.6, 0.1, 0.6), 0.3);
	world.add(sphere(vec3(-4, 0, 1), 1.0, mat3));
